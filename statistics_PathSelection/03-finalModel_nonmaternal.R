...
output_fixed <- file.path(paste0(output_dir,"/nonmaternal_fixed_",
                                 format(Sys.Date(), "%Y%m%d"),
                                 ".csv"))
output_results <- file.path(paste0(output_dir,"/nonmaternal_results_",
                                        format(Sys.Date(), "%Y%m%d"),
                                        ".Rdata"))
# Define parameters for function ----
boot_sample <- 2000
sample_size <- 10
unique_strata <- unique(non_maternal$strata_id)

# Dropped random slope for alder: Running through all the bootstrap samples leads to non-convergence errors and infinite estimates of the alder random slope.
...
# Run model ----
model_results <- iterateBootstrap(data = non_maternal, 
...
# No random slope estimates for this model
...
final_table <-
  ...
  select("Variable Name","Mean Exp Coef","95% CI", 
         "Within-Sample Agreement",-c(lower_95, upper_95))


#### Export tables ----
write_csv(final_table, file=output_fixed)
save(model_results, file = output_results)