# Objectives: Run path selection function for females with calves using conditional logistic regression
...
csv_name <- paste0("clogit_results_calf_",
                   format(Sys.Date(), "%Y-%m-%d"),
                   ".csv")

output_calf <- file.path(output_dir,
                         csv_name)

#### Define parameters ----
unique_ids <- (calf %>% distinct(mooseYear_id))$mooseYear_id
boot_size <- 2000 # Number of bootstrap samples
sample_size <- 10 # 10 controls per case
model_formula <- formula(response ~ 
                           forest_edge_km + 
                           tundra_edge_km + 
                           alnus_mean + 
                           salshr_mean + 
...
#### Run model ----
list_calf <- iterateRun(data = calf, 
                        ids = unique_ids, 
...
df_calf <- data.table::rbindlist(list_calf,use.names=FALSE,idcol=TRUE)
...
# Round values
final_calf <- as.data.frame(cbind(final_calf[,1],sapply(final_calf[,2:5], FUN = round, digits = 2)))
...
#### Export table ----
write_csv(final_calf, file=output_calf)
